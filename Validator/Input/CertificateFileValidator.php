...
class CertificateFileValidator extends InputValidator
{
    ...
    private function validateFile(array $fileData): ConstraintViolationListInterface
    {
        $constraints = new Assert\Collection([
            ...
            'type' => new Assert\EqualTo(value: 'application/x-pkcs12', message: 'Only .p12 files are allowed.'),
            'tmp_name' => new Assert\NotBlank(),
            'error' => new Assert\EqualTo(value: 0, message: 'File upload encountered an error.'),
    ...
    private function validatePassword(string $password): ConstraintViolationListInterface
    {
        $constraints = [
            new Assert\NotBlank(),
            new Assert\Length(max: 128, maxMessage: 'Password cannot be longer than 128 characters.'),
        ];
