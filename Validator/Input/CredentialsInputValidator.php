...
class CredentialsInputValidator extends InputValidator
{
    ...
    private function validateCertificate(array $cert): ConstraintViolationListInterface
    {
        $constraints = new Assert\Collection([
            'certificate' => [
                new Assert\NotBlank(message: 'The certificate field cannot be blank.'),
                new Assert\Length(
                    max: 8192,
            ...
            'passphrase' => [
                new Assert\NotBlank(message: 'The certificate passphrase field cannot be blank.'),
                new Assert\Length(
                    max: 4096,
    ...
    {
        $constraints = [
            new Assert\NotBlank(message: 'The Store UUID field cannot be blank.'),
            new Assert\Regex(
