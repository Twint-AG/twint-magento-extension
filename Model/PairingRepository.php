...
class PairingRepository implements PairingRepositoryInterface
{
    ...
    public function claimCapture(int|string $id): bool
    {
        ...
        // Conditional update: only the first caller flips captured 0 -> 1, so exactly one
        // concurrent monitor process proceeds to capture the payment and send the order email.
        $sql = "UPDATE {$tableName} SET captured = 1 WHERE id = :id AND captured = 0";
