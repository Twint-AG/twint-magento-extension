 ...
 * @internal
 */
class Test_Unit_InvoiceServiceTest extends TestCase
{
    ...
    public function testCreateInvoiceSuccess()
    {
        ...
        $orderMock->shouldReceive('canInvoice')
            ->andReturn(true);
        $orderMock->shouldReceive('prepareInvoice')
            ->andReturn($invoiceMock);
        $orderMock->shouldReceive('getStoreId')
            ->andReturn(1);

        $invoiceMock->shouldReceive('getOrder->setIsInProcess')
            ->with(true)
            ->andReturnSelf();
        $invoiceMock->shouldReceive('setTransactionId')
            ->andReturnSelf();
        $invoiceMock->shouldReceive('register')
            ->andReturnSelf();
        $invoiceMock->shouldReceive('pay')
            ->andReturnSelf();

        $transactionMock->shouldReceive('getTxnId')
            ->andReturn('transaction123');

        $this->invoiceRepositoryMock->shouldReceive('save')
            ->with($invoiceMock);
        ...
        self::assertSame($invoiceMock, $result);
    }
    ...
    public function testCreateInvoiceWithException()
    {
        ...
        $orderMock->shouldReceive('canInvoice')
            ->andReturn(true);
        $orderMock->shouldReceive('prepareInvoice')
            ->andThrow(new LocalizedException(__('Test exception')));

        $this->loggerMock->shouldReceive('error')
            ->with(Mockery::pattern('/Cannot create invoice/'));
    ...
    public function testSendInvoiceMailSuccess()
    {
        ...
        $this->invoiceSenderMock->shouldReceive('send')
            ->with($invoiceMock);
    ...
    public function testSendInvoiceMailWithException()
    {
        ...
        $this->loggerMock->shouldReceive('error')
            ->with(Mockery::pattern('/Exception in Send Mail in Magento/'));
