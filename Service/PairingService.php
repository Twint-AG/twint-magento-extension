...
class PairingService
{
    ...
    protected function recursiveMonitor(
        ...
        if (!$orgPairing->getCaptured() && $tOrder->isSuccessful() && !$orgPairing->isSuccessful()) {
            // Atomically claim the capture. When several monitor processes race on the same
            // pairing, only the one that wins this claim captures the payment and sends the
            // order confirmation email — preventing duplicate emails and invoices.
            if (!$this->pairingRepository->claimCapture($pairing->getId())) {
                return MonitorStatus::fromValues(true, MonitorStatus::STATUS_PAID);
            }
            $pairing->setData('captured', 1);
    ...
    public function cancel(Pairing $pairing): bool
    {
        ...
        if ($pairing->isExpress()) {
            try {
                ...
                $this->logger->error(
                    "PairingService::cancel: TWINT cancel checkin failed {$pairing->getPairingId()}" . $e->getMessage()
                );
                return false;
            }
        } else {
